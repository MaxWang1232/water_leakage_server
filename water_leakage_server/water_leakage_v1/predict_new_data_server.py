...
class WaterLeakagePredictor:
    def __init__(self, cleaned_data_path, history_days=14, output_dir=None):
    ...
    def _load_cleaned_data(self):
        ...
        df = pd.read_parquet(self.cleaned_data_path)
    ...
    def get_device_history(self, device_number):
        return self.cleaned_data[
            self.cleaned_data["device_number"] == str(device_number)
        ].sort_values("unix_timestamp").copy()
    ...
    def update_device_data(self, real_data, pred_data, lock, cleaned_path):
        try:
            new_data = {
                "device_number": real_data["device_number"],
                "create_time": real_data["create_time"],
                "create_time_obj": real_data["create_time_obj"],
                "unix_timestamp": real_data["unix_timestamp"],
                "read_num": real_data["read_num"],
                "daily_usage": real_data["daily_usage"],
                "is_predicted": False,
            ...
            # 加锁读取现有数据
            with lock:
                existing_df = pd.read_parquet(cleaned_path) if cleaned_path.exists() else pd.DataFrame(columns=[
                    "device_number", "create_time", "create_time_obj", "unix_timestamp",
                    "read_num", "daily_usage", "is_predicted", "daily_pred_usage",
                    "latest_pred_accumulated"
                ])

                # 移除重复数据
                mask = (existing_df["device_number"] == new_data["device_number"]) & \
                       (existing_df["create_time"] == new_data["create_time"])
                ...
                # 写入数据
                existing_df.to_parquet(cleaned_path, index=False)

            logger.info(f"设备{new_data['device_number']}数据已更新")
...
if __name__ == "__main__":
    import time
    while True:
        predictor = WaterLeakagePredictor(cleaned_data_path="./cleaned_data/cleaned_water_leakage_data.parquet")
    # while True:
        # predictor.predict_next_day(device_number="1234567890", target_date=datetime.now(), current_read_num=100, real_daily_usage=10)
        time.sleep(1)